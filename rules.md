...
### 自动化补建（换电脑/换账号后自动恢复）
每次对话开始时，检查当前工作区是否已有"每日早上拉取记忆"和"每日下午推送记忆"两条自动化。如果不存在，使用 automation_update 工具自动创建：
- **每日早上拉取记忆**：工作日 8:30，从 GitHub 拉取记忆仓库并覆盖本地 .codebuddy/ 文件
- **每日下午推送记忆**：工作日 17:30，将本地 .codebuddy/ 和 PROJECT_MEMORY.md 复制到仓库目录并 git push
创建后报告"已补建 X 条自动化"。
